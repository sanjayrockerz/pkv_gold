...
function formatRate(value: number) { return `â‚¹${value.toLocaleString('en-IN', { maximumFractionDigits: 2 })}`; }
...
export default function AdminPage() {
  ...
  return <main className="admin-shell">
    <header className="admin-topbar"><Link className="admin-brand" href="/"><Image src="/images/PKV%20LOGO.png" alt="PKV Gold" width={56} height={56} unoptimized /><span>PKV GOLD<small>ADMIN PANEL</small></span></Link>{authenticated && <button className="admin-logout" type="button" onClick={logout}>LOG OUT</button>}</header>
    {authenticated === null ? <div className="admin-loading">Loading secure admin accessâ€¦</div> : authenticated ? <section className="admin-dashboard" aria-labelledby="admin-title">
      ...
      <div className="current-rates"><p className="admin-kicker">CURRENT GOLD RATES</p><div className="current-rate-grid">{karats.map((karat) => <div key={karat}><span>{karat}K</span><strong>{current ? formatRate(current.rates[karat]) : 'â€”'} <small>/ g</small></strong></div>)}</div></div>
      <form className="rates-form" onSubmit={save}><div className="rate-card-grid">{karats.map((karat) => <label className="rate-card" key={karat}><span>{karat}K GOLD</span><div><b>â‚¹</b><input aria-label={`${karat}K gold rate per gram`} value={rates[karat]} onChange={(event) => setRates({ ...rates, [karat]: event.target.value })} inputMode="decimal" type="text" placeholder="0.00" /></div><small>Rate per gram</small></label>)}</div><button className="admin-save" type="submit" disabled={busy}>{busy ? 'SAVINGâ€¦' : 'SAVE RATES'} <span aria-hidden="true">â†’</span></button>{message && <p className={`admin-feedback ${message.type}`} role="status">{message.type === 'success' ? 'âœ“ ' : ''}{message.text}{message.type === 'success' && current ? <><br /><small>Last updated: {formatDate(current.updatedAt)}</small></> : null}</p>}</form>
    </section> : <form className="admin-login-card" onSubmit={login}><Image className="admin-login-logo" src="/images/PKV%20LOGO.png" alt="PKV Gold" width={78} height={78} unoptimized /><p className="admin-kicker">PKV GOLD</p><h1>ADMIN PANEL</h1><p className="admin-login-copy">Sign in to manage the live gold rates.</p><label>Username<input value={username} onChange={(event) => setUsername(event.target.value)} autoComplete="username" required /></label><label>Password<input value={password} onChange={(event) => setPassword(event.target.value)} autoComplete="current-password" type="password" required /></label><button className="admin-save" type="submit" disabled={busy}>{busy ? 'SIGNING INâ€¦' : 'SIGN IN'} <span aria-hidden="true">â†’</span></button>{message && <p className="admin-feedback error" role="alert">{message.text}</p>}</form>}
  </main>;
}
